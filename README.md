...
4. Убедись, что во `frontend/.env` указаны верные урлы
5. Подними проект командой `npm run stage`
6. Логи можно просматривать командой `npm run stage:logs`
7. Погасить можно командой `npm run stage:down`
...
3. Заполни переменные окружения
   ...
   5. Убедись, что во `frontend/.env` указаны верные урлы на нужное доменное имя
   6. Остальные переменные оставь без изменения
4. Пройдите acme-challenge при деплое в первый раз `npm run acme-challenge`
   1. После того как `certbot` успешно завершится, нажми `Ctrl + C`
5. Подними проект командой `npm run prod`
6. Логи можно просматривать командой `npm run prod:logs`
7. Погасить можно командой `npm run prod:down`
